...
######################
    ...
    # Parse sticker:
        if ('sticker' in dataMessage.keys()):
            ...
            self.sticker = self._stickerPacks.getSticker(packId=stickerDict['packId'], stickerId=stickerDict['stickerId'])
    # Parse Quote
        ...
        for result in responseObj['result']['results']:
            if (result['type'] != 'SUCCESS'):
                ...
                if (result['recipientAddress']['number'] != None):
                    added, contact = self._contacts.__getOrAdd__("<UNKNOWN-CONTACT>", result['recipientAddress']['number'])
                else:
                    added, contact = self._contacts.__getOrAdd__("<UNKNOWN-CONTACT>", result['recipientAddress']['uuid'])
                contact.seen(when)
        return when
